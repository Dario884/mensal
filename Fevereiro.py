...
fatxdfev = px.bar(fatfev, x='Dia', y='Faturamento diário', template='plotly_dark', color_discrete_sequence=darkcyan, title=f'Faturamento diário do mês Fevereiro (total = R${fatfev['Faturamento diário'].sum()})',
                 height=600)
lucxdfev = px.bar(fatfev, x='Dia', y='Lucro', template='plotly_dark', color_discrete_sequence=cyan, title=f'Lucro diário do mês Fevereiro (total = R${fatfev['Lucro'].sum():.0f})',
                 height=600)
